...
TEST_CASE( "ScannerConfig1 - from stream", "[class][ScannerConfig]" )
{
	std::ifstream file;
...
TEST_CASE( "ScannerConfig1 - from buffer", "[class][ScannerConfig][buffer]" )
{
	std::ifstream file;
	file.open("./config_files/default.conf", std::ifstream::in);
	...
	ScannerConfig scanner(content.c_str());
	Token t;
